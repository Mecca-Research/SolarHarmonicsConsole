...
- Rendering: Inner planets (Mercury, Venus, Mars) use a custom terminator ShaderMaterial (sun-facing day side, near-dark night side, same soft terminator as Earth); outer planets (Jupiter, Saturn, Uranus, Neptune) use MeshBasicMaterial for 360° full illumination; Pluto uses a view-based limb-darkening ShaderMaterial (disk shades toward the edge from the viewer's perspective, ending with the same tonemapping/colorspace chunks as the built-in materials — raw ShaderMaterial output otherwise skips ACES + sRGB encode and renders far too dark)
...
- Atmosphere: deliberately NO additive glow shells on any body — every one of them read as a transparent ring floating around the planet; atmosphere color lives in the surface textures instead
- Textures: Mercury, Venus, Saturn (body + detailed rings incl. Cassini/Encke/Keeler gaps and F ring), Uranus, Neptune, and Pluto are seeded procedural equirectangular maps modeled on NASA reference photos (MESSENGER enhanced color, Magellan radar, Cassini portrait, Keck, Voyager 2, New Horizons — Pluto has the Sputnik Planitia heart, Cthulhu Macula, gold Lowell Regio, craters and fracture troughs); Mars/Jupiter load CDN maps over procedural fallbacks; Earth uses CDN day/night/water/topology maps. Venus deliberately has no atmosphere glow shell (it read as a ring)
